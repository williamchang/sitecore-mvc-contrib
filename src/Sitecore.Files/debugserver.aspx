...
<div id="frame">
	...
	<%
	var reqServerVariables = Request.ServerVariables;
	%>
	<dl>
		<dt>Request Server Variables Type</dt>
		<dd><%= reqServerVariables.GetType() %></dd>
		<dt>Request Server Variables Count</dt>
		<dd><%= reqServerVariables.Count %></dd>
	</dl>
	<%
	if(reqServerVariables.Count > 0) {
	%>
		<table>
		<thead>
			<tr>
		...
		<tbody>
		<%
		for(int index = 0;index < reqServerVariables.Keys.Count;index += 1) {
		%>
			<tr>
				<td><%= System.Web.HttpUtility.HtmlEncode(reqServerVariables.Keys[index]) %></td>
				<td><%= System.Web.HttpUtility.HtmlEncode(reqServerVariables[index]) %></td>
				<td><%= reqServerVariables[index].GetType() %></td>
			</tr>
		<%
		}
		%>
	...
	<%
	var sysEnvironmentVariables = System.Environment.GetEnvironmentVariables();
	%>
	<dl>
		<dt>System Environment Variables Type</dt>
		<dd><%= sysEnvironmentVariables.GetType() %></dd>
		<dt>System Environment Variables Count</dt>
		<dd><%= sysEnvironmentVariables.Count %></dd>
	</dl>
	<%
	if(sysEnvironmentVariables.Count > 0) {
	%>
		<table>
		<thead>
			<tr>
		...
		<tbody>
		<%
		foreach(System.Collections.DictionaryEntry item in sysEnvironmentVariables) {
		%>
			<tr>
				<td><%= System.Web.HttpUtility.HtmlEncode(item.Key) %></td>
				<td><%= System.Web.HttpUtility.HtmlEncode(item.Value) %></td>
